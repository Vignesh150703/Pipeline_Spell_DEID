...
The API server must be running at http://205.147.102.120:8080 before using these functions.
"""
...
def check_api_health(base_url: str = "http://205.147.102.120:8080") -> bool:
    """
    Check if the Chandra OCR API server is running and healthy.
    
    Args:
        base_url: Base URL of the API server (default: http://205.147.102.120:8080)
...
def process_document(
    ...
    base_url: str = "http://205.147.102.120:8080",
...
if __name__ == "__main__":
    ...
    - It will save the markdown OCR output to `LLM_input.txt`
    """

    input_path = Path("sample-1.pdf")  # Change this if your file is elsewhere
    output_path = Path("LLM_input.txt")

    print("Checking API health...")
    if not check_api_health():
        print("✗ API is not available. Make sure the server is running at http://205.147.102.120:8080")
        exit(1)
