...
def is_valid_phone_number(text: str) -> bool:
    ...
    # Landline with STD (e.g., 0863-3337255 or 040-23456789)
    landline_pattern = re.compile(r"^0?\d{2,4}[- ]?\d{6,8}$")
    # Short local landline (e.g., 3336255)
    short_landline_pattern = re.compile(r"^\d{6,8}$")

    if mobile_pattern.search(raw) or landline_pattern.search(raw) or short_landline_pattern.fullmatch(raw):
        return True
...
def main():
    ...
    for ent in stanford_results:
        ...
        # PHONE_NUMBER: Stanford only, threshold 0.7 (no pattern validation)
        elif etype == "PHONE_NUMBER":
            if ent.get("score", 0.0) < 0.7:
                continue
            entity = sanitize_entity(ent)
            span = (entity.get("start", 0), entity.get("end", 0))
            if any(overlap_fraction(span[0], span[1], ps, pe) > 0.5 for ps, pe in existing_phone_spans):
                continue
